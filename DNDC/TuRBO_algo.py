...
while not state.restart_triggered:  # Run until TuRBO converges
    ...
    df_X_TuRBO.to_csv('df_X_TuRBO.csv', sep=',', index = False, encoding='utf-8')
    
    df_Y_TuRBO = Y_turbo.numpy()
    df_Y_TuRBO = pd.DataFrame(df_Y_TuRBO)
    df_Y_TuRBO.to_csv('df_Y_TuRBO.csv', sep=',', index = False, encoding='utf-8')
    
    with open('gp_model.pkl','wb') as file:
