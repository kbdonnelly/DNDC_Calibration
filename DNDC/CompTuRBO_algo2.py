# -*- coding: utf-8 -*-
"""
Composite TuRBO Optimization Algorithm (Version 2)
...
# Specify model usage (Ensure that necessary scripts are imported):
...
batch_size = 8
n_init = 2*dim
max_evals = 1000 if not SMOKE_TEST else 10
max_cholesky_size = float("inf")  # Always use Cholesky


# This performs evaluation of sensors:
def g(x):
    """This is a helper function we use to unnormalize and evaluate a point"""
    return -simulator_run(x)

# Specifies weights of each function (Check how many outputs come from simulator!)
weights = torch.tensor([5.0,5.0,0.1,0.1])
...
class cTurboState:
    ...
    batch_size: int
    length: float = 0.8
    length_min: float = 0.5**8 # Note: Changed from default of 0.5**7
...
class cTurbo1:
    def __init__(
        ...
        acqf='ts',              # acquisition function type, only support TS for now (need to add EI)
    ...
    def generate_batch(self, model, X, gX, fX):
        ...
        if acqf == "ts":
            ...
            def obj_transform(samples, X): # need to "untransform" the samples generated by the GPs
                return self.obj(samples * Z.std(dim=0) + Z.mean(dim=0))
    ...
    def optimize(self):
        while self.n_evals < self.max_evals:
            ...
            while self.n_evals < self.max_evals and not self.state.restart_triggered:
                ...
                df_g_cturbo = g_cturbo

                df_Y_cturbo = pd.DataFrame(df_Y_cturbo)
                df_X_cturbo = pd.DataFrame(df_X_cturbo)
                df_g_cturbo = pd.DataFrame(df_g_cturbo)

                df_X_cturbo.to_csv('df_X_cturbo1_5N5W01Y.csv', sep=',', index = False, encoding='utf-8')
                df_Y_cturbo.to_csv('df_Y_cturbo1_5N5W01Y.csv', sep=',', index = False, encoding='utf-8')
                df_g_cturbo.to_csv('df_g_cturbo1_5N5W01Y.csv', sep=',', index = False, encoding='utf-8')
                

cturbo1 = cTurbo1(g, weights, dim, n_init, max_evals, batch_size, seed=0)
